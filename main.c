...
void matmatthread(int ldA, int ldB, int ldC, double *A, double *B, double *C, int N, int M, int P, int dbA, int dbB, int dbC, int ntrow, int ntcol)
{
    ...
    int id, thread_row, thread_column;
    #pragma omp parallel private(id, thread_row, thread_column)
    {
...
void matmatdist(MPI_Comm grid, int ldA, int ldB, int ldC, double *A, double *B, double *C, int N, int M, int P, int dbN, int dbM, int dbP, int ntrow, int ntcol, int nprow, int npcol)
{
    ...
    for(k = 0; k != mcm; ++k)
    {
        ...
        matmatthread(M_local, P_local, ldC, A_column, B_row, C, N_local, M_local, P_local, dbM, dbM, dbP, ntrow, ntcol);
    }
}
...
int main(int argc, char **argv)
{
    MPI_Init(&argc, &argv);
    int i;
    for(i = 1024; i <= LIMIT_DIMENSION; i *= 2)
    {
        double *A = NULL;
        double *B = NULL;
        double *C = NULL;
        ...
        if(rank == 0)
            printf("Dimension: %d; Time elapsed: %Lf; Gflops: %Lf\n", i, max_elapsed_time, (2 * pow(i, 3)) / max_elapsed_time / pow(10, 9));
